...
Now checking the `EducationLevel` variable. Education level should be a categorical/factor variable but is loaded as character. We can fix that with simple base R code to mix things up.

```{r}
d3$EducationLevel <- as.factor(d3$EducationLevel)  
skimr::skim(d3)
```
Additionally, checking the `WorkHoursPerWeek` variable -- should be a numeric variable but is loaded as character. We can fix that with simple base R code to mix things up.

```{r}
d3$WorkHoursPerWeek <- as.numeric(d3$WorkHoursPerWeek)  
skimr::skim(d3)
```
